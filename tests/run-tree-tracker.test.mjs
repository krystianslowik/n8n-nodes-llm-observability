...
test('LLM start/end emits one CLIENT span with GenAI + base attributes', () => {
	...
	assert.ok(!('status' in spans[0]), 'successful spans leave status UNSET (omitted) per OTel convention');
});
...
test('capturePrompts=true captures truncated prompt and completion', () => {
	...
	const tracker = new RunTreeTracker(
		// 24-byte budget = 10 content bytes + the 14-byte truncation marker
		{ ...baseConfig, capturePrompts: true, maxPayloadBytes: 24 },
		(s) => spans.push(s),
	);
	const handler = tracker.createHandler();
...
/** Runs one prompt through capture+truncation, returns the exported gen_ai.prompt. */
function capturedPrompt(text, maxPayloadBytes) {
	...
	handler.handleLLMEnd(OPENAI_RESULT, 'run-1');
	return attrMap(spans[0])['gen_ai.prompt'];
}
...
function runToolLoop(configOverrides = {}) {
	...
	handler.handleChatModelStart(
		OPENAI_SERIALIZED,
		[[humanMsg, { content: '', _getType: () => 'ai' }, toolResultMsg('tc1', '3367'), toolResultMsg('tc2', 'found x')]],
		'run-2',
		'agent-run',
	);
	clock.at = 4000;
...
test('tool results echoed into the next model call synthesize tool spans under the shared root', () => {
	const { spans } = runToolLoop({ captureToolIO: true });
	// root, llm-1, tool:calculator, tool:search, llm-2
	assert.equal(spans.length, 5);
	const [root, , toolSpan1, toolSpan2] = spans;
	const toolSpans = [toolSpan1, toolSpan2];
	assert.equal(toolSpan1.name, 'tool:calculator');
	assert.equal(toolSpan2.name, 'tool:search');
	for (const span of toolSpans) {
		...
		assert.equal(span.startTimeUnixNano, '2000000000', 'start == the llmEnd that requested the tool');
		assert.equal(span.endTimeUnixNano, '3000000000', 'end == the chatModelStart that echoed the result');
	...
	assert.ok(String(attrs1['tool.input']).includes('37*91'));
	assert.equal(attrs1['tool.output'], '3367');
	const attrs2 = attrMap(toolSpan2);
	assert.equal(attrs2['gen_ai.tool.name'], 'search');
	assert.equal(attrs2['gen_ai.tool.call.id'], 'tc2');
	assert.equal(attrs2['tool.output'], 'found x');
});

test('synthesized tool spans omit tool.input/tool.output when captureToolIO is off', () => {
	const { spans } = runToolLoop({ captureToolIO: false });
	assert.equal(spans.length, 5);
	for (const span of [spans[2], spans[3]]) {
		const attrs = attrMap(span);
		assert.equal(attrs['gen_ai.tool.name'] !== undefined, true, 'tool name still present');
		assert.equal(attrs['tool.input'], undefined);
		assert.equal(attrs['tool.output'], undefined);
	}
});
...
test('finalize flushes a tool call whose result never reached a later model call', () => {
	...
	handler.handleChatModelStart(OPENAI_SERIALIZED, [[humanMsg]], 'run-1', 'agent-run');
	clock.at = 2000;
	handler.handleLLMEnd(llmEndWithToolCalls([{ id: 'tc1', name: 'calculator', args: { input: '1+1' } }]), 'run-1');
	// no further model call (tool errored mid-flight) — execution end fires finalize
	clock.at = 5000;
	tracker.finalize();
	assert.equal(spans.length, 3, 'root, llm span, flushed tool span');
	const toolSpan = spans[2];
	assert.equal(toolSpan.name, 'tool:calculator');
	assert.equal(toolSpan.parentSpanId, spans[0].spanId);
	...
	assert.equal(attrs['tool.output'], undefined, 'no output was ever observed');
	assert.ok(String(attrs['tool.input']).includes('1+1'));
...
test('pending-tool-call ledger caps at 100: source capped per result, oldest dropped across results', () => {
	...
	handler.handleChatModelStart(OPENAI_SERIALIZED, [[humanMsg]], 'run-1', 'agent-run');
	const calls = Array.from({ length: 150 }, (_, i) => ({ id: `t${i}`, name: `tool-${i}`, args: {} }));
	...
	const more = Array.from({ length: 50 }, (_, i) => ({ id: `u${i}`, name: `tool-u${i}`, args: {} }));
	...
	assert.equal(attrMap(toolSpans[0])['gen_ai.tool.call.id'], 't50', 'oldest ledgered entries were dropped');
...
test('a malformed 10k-entry tool_calls array completes fast and the ledger holds exactly the cap', () => {
	...
	tracker.finalize();
	assert.equal(spans.filter((s) => s.name.startsWith('tool:')).length, 100, 'ledger holds exactly the cap');
});
...
test('a stray handleLLMEnd for an unknown run must not ledger tool calls', () => {
	...
	const tracker = new RunTreeTracker({ ...baseConfig, singleTrace: true }, (s) => spans.push(s));
	const handler = tracker.createHandler();
	handler.handleLLMEnd(llmEndWithToolCalls([{ id: 'tc1', name: 'calculator', args: {} }]), 'never-started');
	tracker.finalize();
...
test('empty-string tool ids never match: two id-less calls flush unmatched instead of cross-matching', () => {
	...
	handler.handleChatModelStart(OPENAI_SERIALIZED, [[toolResultMsg('', 'whose result is this?')]], 'run-2', 'agent-run');
	handler.handleLLMEnd(OPENAI_RESULT, 'run-2');
	assert.equal(spans.filter((s) => s.name.startsWith('tool:')).length, 0, "'' must not match anything");
	tracker.finalize();
	const toolSpans = spans.filter((s) => s.name.startsWith('tool:'));
	...
	for (const span of toolSpans) {
		...
		assert.equal(attrMap(span)['tool.output'], undefined, 'the ambiguous result was never attributed');
	}
});
...
test('non-string tool name/id: span name falls back to unknown, attributes omitted', () => {
	...
	assert.equal(toolSpans[0].name, 'tool:unknown');
...
test('a hostile message property getter cannot abort synthesis or the next LLM run', () => {
	...
	handler.handleChatModelStart(OPENAI_SERIALIZED, [[hostile, toolResultMsg('tc1', '42')]], 'run-2', 'agent-run');
	handler.handleLLMEnd(OPENAI_RESULT, 'run-2');
	assert.ok(tracker.handlerErrors >= 1, 'the hostile getter was counted');
	const toolSpans = spans.filter((s) => s.name.startsWith('tool:'));
	assert.equal(toolSpans.length, 1, 'the well-formed result after the hostile message still matched');
	assert.equal(attrMap(toolSpans[0])['tool.output'], '42');
	assert.equal(spans.filter((s) => s.name.startsWith('llm:')).length, 2, 'run-2 still opened and closed');
});
...
test('wholesale synthesis failure (hostile messages container) still opens the LLM run', () => {
	...
	assert.equal(spans.filter((s) => s.name.startsWith('llm:')).length, 2, 'run-2 span still emitted');
});
...
test('unmatched tool-result messages (no pending entry) are ignored, no span, no throw', () => {
	...
	handler.handleChatModelStart(OPENAI_SERIALIZED, [[humanMsg]], 'run-1', 'agent-run');
	handler.handleLLMEnd(llmEndWithToolCalls([{ id: 'tc1', name: 'calculator', args: {} }]), 'run-1');
	handler.handleChatModelStart(OPENAI_SERIALIZED, [[toolResultMsg('never-requested', 'x')]], 'run-2', 'agent-run');
	handler.handleLLMEnd(OPENAI_RESULT, 'run-2');
	// root + 2 llm spans, no synthesized span for the unmatched result
	assert.equal(spans.filter((s) => s.name.startsWith('tool:')).length, 0);
	assert.equal(tracker.handlerErrors, 0);
});
